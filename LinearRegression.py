...
class LinearRegression():
    def __init__(self, X: np.ndarray[float, int], Y: np.ndarray[float, int], learning_rate: float = 0.1, epochs: int = 1000):
        try:
            if len(X) != len(Y):
                raise ValueError(
                    "LinearRegression: X and Y must be equal length.")
            if len(X) < 3:
                raise ValueError(
                    "LinearRegression: X and Y must be of length of 3 or more.")
            for x in X:
                if not isinstance(x, (float, int)):
                    raise ValueError(
                        "LinearRegression: X must contain float or int only.")
            for y in Y:
                if not isinstance(y, (float, int)):
                    raise ValueError(
                        "LinearRegression: Y must contain float or int only.")
            self.m = float(len(X))
            self.X = X
            self.Y = Y
    ...
    def realtime(self, learning_rate: float = 0.1, figsize: tuple[int | float] = (18, 6), labels: tuple[str, str] = None):
        ...
        plt.subplots_adjust(
            left=0.05, bottom=0.1, right=0.95, top=0.9, wspace=0.2)
        plt.legend()
        plt.show()
